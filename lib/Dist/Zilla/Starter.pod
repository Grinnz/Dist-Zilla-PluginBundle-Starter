...
and build phases of the L<CPAN::Meta::Spec/Phases> in a built distribution (not
to be confused with the Dist::Zilla build that creates this distribution). This
is primarily used to prepare the distribution for testing. In C<[@Starter]>,
L<[MakeMaker]|Dist::Zilla::Plugin::MakeMaker> or the configured installer
...
beginning of the minting process for a new distribution source tree.

=head2 AfterMint

The L<-AfterMint|Dist::Zilla::Role::AfterMint> phase is executed at the end of
the minting process for a new distribution source tree.

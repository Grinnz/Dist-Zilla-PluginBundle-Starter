...
more opinionated structural requirements. The
...
workflow. I will be referencing these bundles throughout this document.

=head1 THE DIST::ZILLA CORE

Dist::Zilla relies almost entirely on its plugin and command system to provide
all functionality, but it is tied together by a few core systems.
...
L<-PluginBundle::PluginRemover|https://metacpan.org/pod/Dist::Zilla::Role::PluginBundle::PluginRemover> and
...
to be confused with the L<Dist::Zilla> build that creates this distribution).
This is primarily used to prepare the distribution for testing. In
C<[@Starter]>, L<[MakeMaker]|Dist::Zilla::Plugin::MakeMaker> or the configured
installer plugin handles this phase.

...
lists the dependencies of the distribution itself. The output of both can be
...
One more common task is to copy updated versions of generated files from the
distribution build to the source tree, such as generated F<README> files or the
F<Makefile.PL>/F<Build.PL> and F<META.json> so the source tree can be
installed. Often this is done whenever the distribution is built with
...
same command and plugin system to provide a minting tool. This allows authors
to use or provide a minting profile which defines how to create a new project
and set up the basic framework automatically to their preferences. The
...
L<< C<dzil new>|Dist::Zilla::App::Command::new >> command, which defaults to using
the local default profile in C<~/.dzil/profiles/default> if present and
otherwise a default C<[@Basic]> profile. The C<-p> option can be used to
specify the profile name to use, and the C<-P> option can specify an installed
minting profile as the provider. The default profile or provider can be set in
the C<~/.dzil/config.ini> configuration as well as documented for C<dzil new>.
